...
import createArchive from '../helpers/zip.js'
...
export async function build (dataFilePath, presskit, {
  ...
  const imagesArchive = await createArchive('images.zip', assetsTarget, assetsSource, images.screenshots)
  const logosArchive = await createArchive('logos.zip', assetsTarget, assetsSource, images.logos)

  // Absolute path for images (og:image)
  presskit.path = pageFolder.split('/').pop()
  ...
  const html = template({
    ...
    hasZip: config.commands.build.zip,
    imagesArchive,
    logosArchive,
